  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name = local.kafka_rest.name
          image = local.kafka_rest.image

          dynamic "port" {
            for_each = local.kafka_rest.ports
          ...
          dynamic "env" {
            for_each = local.kafka_rest.envs
          ...
          resources {
            limits {
              cpu = local.kafka_rest.limits.cpu
              memory = local.kafka_rest.limits.memory
            }

            requests {
              cpu = local.kafka_rest.requests.cpu
              memory = local.kafka_rest.requests.memory
            }
          }
